...
const MODEL: &str = include_str!("../assets/hopper.xml");

/// Gymnasium-compatible `Hopper-v5` with the default configuration.
pub struct HopperV5 {
    core: MujocoCore,
...
impl HopperV5 {
    ...
    pub fn new(
        ...
        #[builder(default = (-100.0, 100.0))] healthy_state_range: (f64, f64),
        #[builder(default = (0.7, f64::INFINITY))] healthy_z_range: (f64, f64),
        #[builder(default = (-0.2, 0.2))] healthy_angle_range: (f64, f64),
        ...
        validate_range("healthy_state_range", healthy_state_range)?;
    ...
    fn is_healthy(&self) -> bool {
        ...
        let state_is_finite_and_bounded = qpos[2..].iter().chain(self.core.qvel()).all(|value| {
            value.is_finite()
                && self.healthy_state_range.0 < *value
                && *value < self.healthy_state_range.1
        });
        state_is_finite_and_bounded
